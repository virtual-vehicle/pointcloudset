...
\autoref{fig:dataset} illustrates the structure of the a `Dataset` including import, export possibilities. A Dataset consist of many point clouds which can be accesses like list elements in Python. Alternativly a `PointCloud` object can also be created from files, as illustrated in \autoref{fig:pointcloud}.
