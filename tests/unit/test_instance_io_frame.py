...
def test_from_pyntcloud(testlas1: Path):
    ...
    frame = pointcloudset.Frame.from_instance("pyntcloud", pyntcloud_data)
    check.is_instance(frame, pointcloudset.Frame)
...
def test_from_open3d(testframe_mini_real: Frame):
    ...
    frame = pointcloudset.Frame.from_instance("open3d", open3d_data)
...
def test_from_dataframe(testframe_mini_df: pd.DataFrame, testframe_mini: Frame):
    frame = pointcloudset.Frame.from_instance("DataFrame", testframe_mini_df)
