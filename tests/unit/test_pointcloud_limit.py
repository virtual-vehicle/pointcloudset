...
def test_frame_limit(testframe_mini: PointCloud):
...
def test_frame_limit2(testframe_mini: PointCloud):
    with pytest.raises(KeyError):
        testframe_mini.limit("wrong", minvalue=0.0, maxvalue=200)


def test_frame_limit3(testframe_mini: PointCloud):
    with pytest.raises(ValueError):
        testframe_mini.limit("wrong", minvalue=2000, maxvalue=0)


def test_frame_limit4(testframe_mini: PointCloud, testframe_mini_df):
...
def test_frame_limit5(testframe_mini: PointCloud, testframe_mini_df):
    np.testing.assert_array_equal(
...
def test_frame_limit_chaining(testframe_mini: PointCloud, testframe_mini_df):
...
def test_frame_limit_chaining2(testframe_mini: PointCloud):
